...
def _get_connection_link_or_none(token):
    ...
    try:
        return ConnectionLink.objects.select_related(
...
def _build_connection_redirect_uri(request, platform):
    ...
    return request.build_absolute_uri(
        reverse("onboarding:oauth_callback", kwargs={"platform": platform})
    )
...
def create_link(request, workspace_id):
    ...
    link_url = request.build_absolute_uri(
        reverse("onboarding:connection_page", kwargs={"token": link.token})
    )
...
def revoke_link(request, workspace_id, link_id):
    ...
    link = get_object_or_404(
        ConnectionLink.objects.for_workspace(workspace_id), id=link_id
    )
...
def send_link_email(request, workspace_id, link_id):
    ...
    text_content = render_to_string(
        "onboarding/email/connection_link.txt", context
    )
    html_content = render_to_string(
        "onboarding/email/connection_link.html", context
    )
...
def connection_page(request, token):
    ...
    connected_accounts = {
        usage.social_account.platform: usage.social_account
        for usage in connected_usages
    }
...
def connection_oauth_start(request, token):
    ...
    state = _sign_connection_link_state(
        link.workspace_id, platform, token, nonce
    )
...
def connection_oauth_callback(request, platform):
    ...
    if error:
        ...
        session_data = request.session.pop(
            CONNECTION_LINK_OAUTH_SESSION_KEY, {}
        )
    ...
    try:
        ...
        logger.exception(
            "Connection link OAuth callback failed for %s", platform
        )
...
def connection_done(request, token):
    ...
    connected_count = ConnectionLinkUsage.objects.filter(
        connection_link=link
    ).count()
    connected_accounts = (
        ConnectionLinkUsage.objects.filter(connection_link=link)
...
def connection_bluesky_connect(request, token):
    ...
    if not handle or not app_password:
        request.session["connection_link_error"] = (
            "Handle and app password are required."
        )
        return redirect("onboarding:connection_page", token=token)
    ...
    try:
        provider = _get_provider_for_platform(
            PlatformCredential.Platform.BLUESKY, org.id
        )
...
def connection_mastodon_start(request, token):
    ...
    try:
        registration = MastodonAppRegistration.objects.get(
        ...
        try:
            ...
            redirect_uri = _build_connection_redirect_uri(
            ...
            logger.exception(
                "Mastodon app registration failed for %s", instance_url
            )
            request.session["connection_link_error"] = (
